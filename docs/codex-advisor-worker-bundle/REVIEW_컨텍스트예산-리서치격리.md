...
> ⚠️ **이 문서는 2026-07-27 시점의 설계 검토 기록이다 — 현행 명세가 아니다.**
> 이후 구현·개정됐고, 특히 **임계치가 바뀌었다**: 본문의 고정 델타 `51,133`/`59,000` 은
> **폐기**됐고 현행은 **창 비례 + 창 티어별 비율**이다(소형 창 15%/20%, 대형 창 40%/55%, 2026-08-08).
> 고정값은 200k 창에서 역산한 것이라 1M 창에서 창의 6% 지점에 울렸다.
> 아래 본문의 수치·"미구현" 표기는 **그 시점의 판단을 보존한 것**이며 의도적으로 고치지 않았다
> (사후 개작하면 실측 근거와 Red-Green 기록이 거짓이 된다).
> **현행 동작은 `README_조언자-작업자-전략.md` 와 `HANDOFF.md` 를 보라.**
>
> 대상 번들: `docs/codex-advisor-worker-bundle/` (조언자–작업자–Codex 검증)
> 작성일: 2026-07-27 · 상태: **검토(설계 제안)** — 당시 미구현
